...
*This plan is already written, except for the research question. Read it before you fill that in:
everything below is a decision someone had to make before seeing the data, and every one of them
is fixed for today so that your numbers match everybody else's.*

Anything that is not written here is exploratory.

## Research question
<!-- One sentence, in your own words: what do you want to know? The rest of this plan answers it. -->
...
## Date & git tag
Written before any raw data was downloaded. Commit this file, then `git tag pap-v1 && git push --tags`;
`git log` is the record of when the plan existed.
